...
function addOrderItem(item) {
    const productId = newProductList.getSelectedKey();

    const emptyItem = {
        products_id: item.id,
        products_productId: productId, //item.products_id,
        products_no: item.no,
        products_description: item.description,
        products_type: item.no,
        products_baseUnitOfMeasure: item.baseUnitOfMeasure,
        average_amount: item.amount,
        average_amount_conf: item.amount,
    };
    let existingData = modelproductsTable.getData();

    if (!Array.isArray(existingData)) {
        existingData = [];
    }
    existingData.push(emptyItem);
    modelproductsTable.setData(existingData);
...
function getTotWeightByTruck(truck) {
    return modelAppointments
        ...
        .appointments.filter(function (app) {
            return (
                app.truck === truck && app.startDateTime &&
